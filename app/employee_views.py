...
class employeeView(View):
    ...
    def list(self, request):
        ...
        transactions = employee.objects.filter(filter_params) if filter_params else employee.objects.all()
        ...
        return self.context, 'app/employee/list.html'

    def edit(self, request, pk):
        print("Ram its editing")
        transaction = self.get_object(pk)
        self.context['employee'] = transaction
        self.context['form'] = employeeForm(instance=transaction)
        return self.context, 'app/employee/edit.html'
    ...
    def add(self, request): 
        if request.method == "POST":
            ...
            self.context['form'] = employeeForm()
            return self.context, 'app/employee/add.html'
    ...
    def edit_row(self, pk):
        ...
        form = employeeForm(instance=transaction)
        context = {'instance': transaction, 'form': form}
        return render_to_string('app/employee/edit_row.html', context)

    """ Common methods """

    def get_object(self, pk):
        transaction = get_object_or_404(employee, id=pk)
        return transaction

    def get_row_item(self, pk):
        transaction = self.get_object(pk)
        edit_row = render_to_string('app/employee/edit_row.html', {'instance': transaction})
        return edit_row

    def update_instance(self, request, pk, is_urlencode=False):
        transaction = self.get_object(pk)
        form_data = QueryDict(request.body) if is_urlencode else request.POST
        form = employeeForm(form_data, instance=transaction)
        if form.is_valid():
            form.save()
            print("Sucess")
            if not is_urlencode:
